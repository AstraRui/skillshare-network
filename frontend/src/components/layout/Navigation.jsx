      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-3 rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 shadow-2xl backdrop-blur-xl md:flex-row md:items-center md:justify-between md:gap-0 md:px-6">
        <div className="flex items-center justify-between gap-3">
          ...
          {!isAuthenticated ? (
            ...
            <NavLink to="/profile" className={`${profileButtonClass} md:hidden`}>
        ...
        {!isAuthenticated ? (
          <div className="hidden items-center justify-end md:flex md:shrink-0">
            <button type="button" onClick={openAuthModal} className={profileButtonClass}>
              <div className="flex size-7 items-center justify-center rounded-lg bg-indigo-600/80 text-indigo-200">
                <User size={14} strokeWidth={2.5} aria-hidden />
              </div>
              <div className="hidden pr-2 text-left lg:block">
                <p className="text-[10px] font-black leading-none text-white">Войти</p>
          ...
          <div className="hidden items-center justify-end space-x-4 md:flex md:shrink-0">
            <NavLink to="/profile" className={profileButtonClass}>
              <div className="flex size-7 items-center justify-center rounded-lg bg-indigo-600 text-[10px] font-bold text-white">
                {userInitials}
              </div>
              {email ? (
